...
class Drawing:
    ...
    def draw_player(self, sc, player_pos, cur_player_pos):
        elf_image1 = pygame.image.load()
        x, y = player_pos
        for x, y in cur_player_pos:
            screen.blit(elf_image1, (x+TILE//2, y+TILE//2) )
...
def generate_about_game(current_screen):
    ...
    while running_about_game:
        ...
        pygame.display.flip()
def generate_training(current_screen, player_position = player_position, last_player_position=last_player_position, matrix_walls_map=matrix_walls_position_map, matrix_player_vision_map=np.ones((map_height, map_width))):
    ...
    while running_training:
        ...
        for event in pygame.event.get():
            ...
            if (stop_training == False):
                print_text(message="Чтобы пропустить обучение, нажмите Backspace", x=400, y=10, font_color=WHITE)
        ...
        pygame.display.flip()
        clock.tick(FPS)
def generate_first_level(current_screen="first level", player_position = player_position, last_player_position=last_player_position, matrix_walls_map=matrix_walls_position_map, matrix_player_vision_map=np.ones((map_height, map_width))):
...
def level_layers_generate (map_width, map_height, vis_distance, map_name, file ):
    ...
    matrix_dec_pos_map = np.ones((map_height, map_width))
    matrix_dec_pos_map[18, 18] = 4
    ''''''
    ...
    matrix_NPC_pos_map = np.ones((map_height, map_width))

...
background_image_to_training_level  = pygame.image.load("pictures/training_level.jpg")
...
background_image_to_generate_end_of_game = pygame.image.load("pictures/end_of_game.jpg")
...
def print_text( message, x, y, font_color, font_type= pygame.font.Font('fonts/Mediteran.ttf', 12), sc =pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.NOFRAME)):
...
class Hero:
    ...
    def __init__(self, name, healthpoints, damage, armor, attack_range, movement_speed, vision_distance, player_position):
        self.name = name
    ...
    def take_damage(self, hit):
        ...
        if injury > 0:
            ...
            if self.healthpoints <= 0:
                ...
                (cur_NPC_pos_map, matrix_NPC_pos_map, set_NPC_coordinates) = update_NPC_level_layer(matrix_NPC_pos_map, dead_NPC_positions)
                self.is_alive = False
                self.healthpoints = 0
...
class NPC:
    def __init__(self, name, healthpoints, damage, armor, attack_range, movement_speed, vision_distance, NPC_position):
        self.name = name
    ...
    def take_damage(self, hit):
        ...
        if injury > 0:
            ...
            if self.healthpoints <= 0:
                self.is_alive = False
                self.healthpoints = 0
        print('enemy_HP = ', self.healthpoints)
...
class StatusBar:
    ...
    def draw(self, screen):
        ...
        # Отрисовка текста
        print_text(message=self.text, x = self.x, y = self.y, font_color = STATUS_BAR_TEXT_COLOR, font_type =pygame.font.Font('fonts/RAINFALL Black.otf', 12))
...
back_exit_button.font = pygame.font.Font('fonts/AlgerianBasD [Rus by me] Regular.otf', 26)
real_exit_button = Button("Выйти", (560, 350))
real_exit_button.font = pygame.font.Font('fonts/AlgerianBasD [Rus by me] Regular.otf', 26)
...
def level_layers_initialization (map_width, map_height, vis_distance, map_name, file,player_position = player_position):
    ...
    last_player_position = player_position
    matrix_player_vis_map[(player_position[0] - vis_distance):(player_position[0] + vis_distance+1), (player_position[1] - vis_distance): (player_position[1] + vis_distance+1):] = 0
    matrix_player_vis_map[(player_position[0] - 1):(player_position[0] + 1 + 1), (player_position[1] - 1): (player_position[1] + 1 + 1):] = 0
    matrix_player_vis_map[(player_position[0] - 2):(player_position[0] + 2 + 1), (player_position[1] - 2): (player_position[1] + 2 + 1):] = 0
...
def choose_window_generation(current_screen):
    match current_screen:
        ...
        case "end_of_game":
            ...
            current_screen_inside_choose_window_generation = generate_end_of_game(current_screen="end_of_game")
            return (current_screen_inside_choose_window_generation)
...
def generate_menu(current_screen):
    ...
    while running_menu:
        '''
        screen.blit(pygame.image.load("pictures.nemeron_gates.jpg"), (-100, 0))
...
def generate_exit(current_screen):
    ...
    while running_exit:
        ...
        if current_screen == "exit":
            ...
            print_text('Вы уверены, что хотите выйти из игры?', 50, 200, font_color=WHITE, font_type= pygame.font.Font('fonts/AlgerianBasD [Rus by me] Regular.otf', 32))
...
def generate_end_of_game(current_screen):
    ...
    while running_exit:
        ...
        # '''
        text_introduction = ["Поздравляем!","Ты прошёл игру!"]
        movement = 0
        for i in text_introduction:
            text_surf_1 = font.render(i, True, WHITE)
        ...
        if current_screen == "exit":
            pygame.display.set_caption('Подтверждение выхода')
            print_text('Вы уверены, что хотите выйти из игры?', 50, 200, font_color=WHITE,
                       font_type=pygame.font.Font('fonts/AlgerianBasD [Rus by me] Regular.otf', 32))
            back_exit_button.draw(screen)
            real_exit_button.draw(screen)
        elif current_screen == "menu":
            generate_menu(current_screen="menu")
